    ...
    class WorkoutStorage: ObservableObject {
        ...
        init(achievementManager: AchievementManager = AchievementManager.sharedInstance,
             profileManager: UserProfileManager? = nil) {
        ...
        func save(workout: Workout) {
            ...
            profileManager?.updateStreak(workout.date)
            // ✅ Evaluate achievements using injected instance
            achievementManager?.evaluateAchievements(using: history,currentStreak: profileManager?.profile.currentStreak ?? 0) { newAchievement in
        ...
        func load() {
            ...
            // If we're already listening for this user, do nothing.
            if listeningForUserID == userID, listener != nil { return }
            ...
            listener = db.collection("users")
                ...
                .addSnapshotListener { [weak self] snapshot, error in
                    ...
                    let newHistory: [Workout] = snapshot?.documents.compactMap { doc -> Workout? in
                        guard var workout = Workout.fromDictionary(doc.data()) else { return nil }
